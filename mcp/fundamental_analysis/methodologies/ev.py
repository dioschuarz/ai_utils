...
class EVMethodology(BaseMethodology):
    """
    Enterprise Value Multiple (EV) valuation methodology.
    Values company using comparable peer EV/EBITDA multiples.
    """
    ...
    def calculate(
        ...
        if ebitda <= 0:
            return self.create_diagnostic_result(
                ...
                explanation=f"EV methodology requires positive EBITDA/operating cash flow base. EBITDA: {ebitda:,.0f}.",
                triggering_metrics={"ebitda": ebitda},
                analytical_implication="Traditional EV valuation is not applicable to banks or unprofitable companies lacking positive EBITDA.",
                baseline_metrics={
                    "ebitda": ebitda,
        ...
        if primary_equity_value <= 0:
            return self.create_diagnostic_result(
                ...
                analytical_implication="The company's debt burden is larger than the projected enterprise value derived from EBITDA multiples, leaving no equity value.",
                baseline_metrics={
                    "enterprise_value": primary_enterprise_value,
                    "total_debt": total_debt,
                    "total_cash": total_cash,
        ...
        assumptions.append(f"BR EV Valuation: {intrinsic_value_br:.2f} (Verdict: {verdict_br})")
        assumptions.append(f"US EV Valuation: {intrinsic_value_us:.2f} (Verdict: {verdict_us})")
